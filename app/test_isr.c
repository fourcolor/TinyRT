...
static trt_sem_t sem_isr_signal;
...
static void sem_isr_waiter_task(void *arg)
{
    ...
    timer_setup(&sem_isr_timer, sem_isr_callback, 0);
    timer_start(&sem_isr_timer, 750, 750);
...
void app_test_isr_start(void)
{
    trt_sem_init(&sem_isr_signal, 1, 0);
